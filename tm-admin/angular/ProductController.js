...
var appp = angular.module('ProductApp', ['ngRoute', 'flow', 'ngAnimate', 'ngProgress', 'ngTagsInput', 'datatables']);
...
appp.controller('ProductController', function ($window, $scope, $rootScope, $http, DataService, fileUpload, $timeout, ngProgressFactory) {
    ...
    var GetAllProducts = function () {
        ...
        $http({
            method: 'POST',
            url: '/AdminOperation.asmx/GetProduct',
            data: data,
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        }).then(function (success) {
    ...
    $scope.SaveAttributeMaster = function () {
        ...
        var _atm = {
            Id: $scope.AttributeMasterList.length + 1,
    ...
    $scope.SaveProduct = function (Product) {
        ...
        //  console.log($scope.ddlCategoryName);
        ...
        var StockQuantity = $scope.StockQuantity;
        var Price = $scope.Price;
        var OldPrice = $scope.OldPrice;
        var ProductCost = $scope.ProductCost;
        var SKU = $scope.SKU;
        //   var GTIN = $scope.GTIN;
        var ShowOnPage = $scope.ShowOnPage;
        ...
        //var AttributeValueList = JSON.stringify($scope.AttributeValueList);
        //var MainAttributeList = JSON.stringify($scope.Attribute);
        //var PRODUCT_VARIANT = JSON.stringify($scope.VariantList);
        //console.log(JSON.stringify(AttributeMasterList));
        ...
        var message = "";
        //for (var i = 0; i < $scope.ddlCategory.length; i++) {
        //    var fruitId = $scope.ddlCategory[i];
        //    var fruitName = "";
        ...
        var SizeChartImage = "";
        var FileUploadCtrl_SizeChart = document.getElementById("SizeChartImg");
        var FilePath_SizeChart = FileUploadCtrl_SizeChart.value;
        var Ext_SizeChart = FilePath_SizeChart.substring(FilePath_SizeChart.lastIndexOf('.') + 1).toLowerCase();
            ...
            $http(request)
                .success(function (respond) {
                    ...
                    var data = $.param({
                        ...
                        PRODUCT_EXPIRY: $scope.PRODUCT_EXPIRY,
                        BRAND_ID: $scope.ddlBrand,
                        MATERIAL_ID: $scope.ddlMaterial,
                    ...
                    $http({
                        method: 'POST',
                        url: '/AdminOperation.asmx/SaveProduct',
                        data: data,
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    }).then(function (success) {
                        console.log(success);
                        ...
                        $(".notifications").attr("data-custommessage", "Product Saved Successfully ! We are Redirecting to Add Multiple Images of This Product/Attribute");
                        $('.notifications').trigger('click');
