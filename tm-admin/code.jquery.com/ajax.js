...
function AddControl() {
    ...
    html = '<tr>';
    html += '<td> <button type="button" onclick="DeleteRow(this)" class="delete btn btn-sm bgm-red"><i class="zmdi zmdi-close"></i></button></td>';
    ...
    //html += '<td><input type="hidden" data-type="productCode" name="itemNo[]" id="itemNo_' + i + '" class="form-control autocomplete_txt" autocomplete="off">';
    //    '<input type="text" data-type="productName" name="itemName[]" id="itemName_' + i + '" class="form-control autocomplete_txt" autocomplete="off"></td>';
    //html += '<td><input type="text" name="unit[]" id="unit_' + i + '" class="form-control " autocomplete="off" ondrop="return false;" onpaste="return false;"></td>';
    html += '<td><select class="form-control myselect"><option value="0">Select Units</option><option value="10">NOS</option></td>';
    ...
    html += '<td><input type="text" name="price[]" value="0.00" id="price_' + i + '" class="price form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
    html += '<td><input type="text" name="tax[]" value="0.00" id="tax_' + i + '" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
    html += '<td><input type="text" name="total[]" value="0.00" id="total_' + i + '" class="form-control totalLinePrice" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
    html += '</tr>';
    $('table').append(html);
    $('#itemName_' + i).focus();
    i++;
...
$(".addmore").on('click', function () {
    html = '<tr>';
    html += '<td> <button type="button" class="delete btn btn-sm bgm-red waves-effect"><i class="zmdi zmdi-close"></i></button></td>';
    //html += '<td><input type="text" data-type="productCode" name="itemNo[]" id="itemNo_' + i + '" class="form-control autocomplete_txt" autocomplete="off"></td>';
    html += '<td><input type="hidden" data-type="productCode" name="itemNo[]" id="itemNo_' + i + '" class="form-control autocomplete_txt" autocomplete="off"><input type="text" data-type="productName" name="itemName[]" id="itemName_' + i + '" class="form-control autocomplete_txt" autocomplete="off"></td>';
    //html += '<td><input type="text" name="unit[]" id="unit_1" class="form-control " autocomplete="off" ondrop="return false;" onpaste="return false;"></td>';
    html += '<td><select class="form-control myselect"><option value="10">NOS</option></select></td>';
    html += '<td><input type="text" name="quantity[]" id="quantity_' + i + '" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
    html += '<td><input type="text" name="price[]" id="price_' + i + '" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
    //html += '<td><input type="text" name="tax[]" value="0.00" id="tax_1" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
        ...
        source: function (request, response) {
            $.ajax({
                ...
                success: function (data) {
                    console.log(data);
                    var array = $.map(data.d, function (item) {
                        return {
                            label: item.split('|')[0],
                            value: item.split('|')[0],
                            data: item
...
$(document).on('change keyup blur', '.changesNo', function () {
    ...
    var finalrate = $('#finalrate_' + id[1]).val();
    //$('#finalrate_' + id[1]).val(price)
    var disc_rs = price - finalrate;
    $('#discrs_' + id[1]).val(disc_rs.toFixed(2));
    $('#discperc_' + id[1]).val(parseFloat((disc_rs * 100) / price).toFixed(2));

    //discrs_1

    if (quantity != '' && finalrate != '') $('#total_' + id[1]).val((parseFloat(finalrate) * parseFloat(quantity)).toFixed(2));
    calculateTotal();
});
// Calculate Total when Percentage changed
$(document).on('change keyup blur', '.changesNo_discperc', function () {
    id_arr = $(this).attr('id');
    id = id_arr.split("_");
    quantity = $('#quantity_' + id[1]).val();
    price = $('#price_' + id[1]).val();
    var finalrate = $('#finalrate_' + id[1]).val();
    var discperc = $('#discperc_' + id[1]).val();
    //$('#finalrate_' + id[1]).val(price)
    var disc_rs = price - finalrate;

    $('#finalrate_' + id[1]).val((price - ((price * discperc) / 100)).toFixed(2));
    //discrs_1
    $('#discrs_' + id[1]).val(((price * discperc) / 100).toFixed(2));

    if (quantity != '' && finalrate != '') $('#total_' + id[1]).val((parseFloat(finalrate) * parseFloat(quantity)).toFixed(2));
    calculateTotal();
});
// Calculate Total when Discount Rs Changes 
$(document).on('change keyup blur', '.changesNo_discrs', function () {
    id_arr = $(this).attr('id');
    id = id_arr.split("_");
    quantity = $('#quantity_' + id[1]).val();
    price = $('#price_' + id[1]).val();
    var finalrate = $('#finalrate_' + id[1]).val();
    var discrs = $('#discrs_' + id[1]).val();
    //$('#finalrate_' + id[1]).val(price)
    var disc_rs = price - discrs;
    $('#finalrate_' + id[1]).val(disc_rs.toFixed(2));
    $('#discperc_' + id[1]).val(((discrs * 100) / price).toFixed(2));
    //discperc_
    if (quantity != '' && finalrate != '') $('#total_' + id[1]).val((parseFloat(finalrate) * parseFloat(quantity)).toFixed(2));
    calculateTotal();
});
...
function calculateTotal() {
    subTotal = 0; total = 0, mrp_total = 0, disc_total = 0;
    $('.totalLinePrice').each(function () {
        if ($(this).val() != '') subTotal += parseFloat($(this).val());
    });
    $('.price').each(function () {
        if ($(this).val() != '') mrp_total += parseFloat($(this).val());
    });
    $('.discrs').each(function () {
        if ($(this).val() != '') disc_total += parseFloat($(this).val());
    });
    $('#NetPayable').text(subTotal.toFixed(2));
    tax = $('#tax').val();
    ...
    $('#TotalSales').text(mrp_total.toFixed(2));
    $('#TotalDiscount').text(disc_total.toFixed(2));
    calculateAmountDue();
}
...
specialKeys.push(8, 46, 39, 37, 36, 35); //Backspace
